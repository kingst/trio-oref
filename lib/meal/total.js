...
function recentCarbs(opts, time) {
    ...
    var lastCarbTime = 0;
    if (!treatments || treatments.length == 0 || !glucose_data || glucose_data.length == 0) return {};

    //console.error(glucose_data);
    // We need to set clock on iob_inputs to make sure
    // that we can replay the IoB calculation, otherwise
    // IoB uses the current time of day
    var iob_inputs = {
