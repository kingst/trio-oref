...
// Initialize and calculate variables used for predicting BGs
    ...
    // BUG: The original JS was missing the maxCI check for uci
    if (uci > maxCI) {
        uci = maxCI;
    }
    ...
    // BUG: We need to limit the remaining carb absorption time against the max set
    // by the user, so this min check accomplishes this
    remainingCATime = Math.min(remainingCATime, profile.maxMealAbsorptionTime);
