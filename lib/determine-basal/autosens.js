...
var find_insulin = require('../iob/history').calcTempTreatments;
...
//console.error("skipping:",glucose_data[i].glucose,glucose_data[i-1].glucose);
    ...
    for (i=3; i < bucketed_data.length; ++i) {
        ...
        // BUG: Add precision to rounding
        var bgiPrecise = Math.round(( -iob.activity * sens * 5 )*10000000)/10000000;
        var bgi = Math.round(bgiPrecise*100)/100;
        bgi = bgi.toFixed(2);
        //console.error(delta);
        var deviation;
        if (isNaN(delta) ) {
        ...
        debugInfo.push({bgi: bgi, iobActivity: iob.activity, deltaGlucose: delta, deviation: deviation, stateType: type, mealCOB: mealCOB, absorbing: absorbing, mealCarbs: mealCarbs, mealStartCounter: mealStartCounter});
