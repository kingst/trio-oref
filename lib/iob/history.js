...
function splitAroundSuspends (currentEvent, pumpSuspends, firstResumeTime, suspendedPrior, lastSuspendTime, currentlySuspended) {
    ...
    if (suspendedPrior && (currentEvent.date < firstResumeDate)) {
        if ((currentEvent.date+currentEvent.duration*60*1000) < firstResumeDate) {
            ...
            // BUG: The original JS forgot to set timestamp
            currentEvent.timestamp = currentEvent.started_at;
...
function calcTempTreatments (inputs, zeroTempDuration) {
    ...
    for (var i=0; i < pumpHistory.length; i++) {
        ...
        // BUG: we should skip any suspend / resume events
        // in the future
        var currentTime = new Date(tz(current.timestamp));
        if (currentTime > now) {
            continue;
        }
    ...
    // BUG: Conceptually the JS code intends to drop subsequent suspend
    // and resume events when they appear consecutively, but it double
    // counts suspends so we will manually remove these
    if (pumpSuspends.length >= 2) {
        var suspendAndResumes = _.sortBy(pumpSuspends.concat(pumpResumes), 'date');
        pumpSuspends = [];
        if (suspendAndResumes[0].type == "PumpSuspend") {
            pumpSuspends.push(suspendAndResumes[0]);
        }
        for (var index = 1; index < suspendAndResumes.length; index++) {
            if (suspendAndResumes[index].type == "PumpSuspend") {
                if (suspendAndResumes[index-1].type != suspendAndResumes[index].type) {
                    pumpSuspends.push(suspendAndResumes[index]);
                }
            }
        }
    }
    ...
    // BUG FIX: This zero temp logic used to be within the pumpHistory
    // for loop, so we'd get one for each event. The bug is when
    // there are no events in the pumpHistory and the JS omitted it
    ...
    if (suspend_zeros_iob) {
        ...
        // BUG: We got rid of the hard coded 8 dia and increased
        // the duration of the suspend to cover the full pump history
        // so that this function produces a correct output for meal
        // and autosens
        // NOTE: We're assuming that the max pump history is 36 hours
        var max_pump_history_ago = now.getTime() - 36*60*60*1000;
        var firstResumeStarted = new Date(firstResumeTime);
        var firstResumeDate = firstResumeStarted.getTime()

        if (suspendedPrior && (max_pump_history_ago < firstResumeDate)) {

            var suspendStart = new Date(max_pump_history_ago);
            ...
            var zTempBasal = [{
               ...
               // anywhere.
                _type: 'SuspendBasal',
                rate: 0,
                duration: (firstResumeDate - max_pump_history_ago)/60/1000,
        ...
        if (currentlySuspended) {
            ...
            var zTempBasal = [{
                _type: 'SuspendBasal',
                rate: 0,
                // FIXME: comparing a date object and time
    ...
    for (i=0; i < splitHistory.length; i++) {
        ...
        if (currentItem.duration > 0) {
            ...
            if (!_.isEmpty(profile_data.basalprofile)) {
                // BUG: suspends don't have timestamp defined, so we fall back to date
                var basalDate;
                if (currentItem.timestamp) {
                    basalDate = new Date(currentItem.timestamp);
                } else {
                    basalDate = new Date(currentItem.date);
                }
                currentRate = basalprofile.basalLookup(profile_data.basalprofile, basalDate);
            }
