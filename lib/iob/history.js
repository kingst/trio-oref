...
function calcTempTreatments (inputs, zeroTempDuration) {
    ...
    if (suspend_zeros_iob) {
        ...
        // resume
        // BUG: We got rid of the hard coded 8 dia
        var profileDia = profile_data.dia;
        if (typeof profileDia === 'undefined') {
            profileDia = 10; // default to 10 if not set
        }
        if (profileDia < 5) {
            profileDia = 5;
        }
        var max_dia_ago = now.getTime() - profileDia*60*60*1000;
        // END BUG FIX
        var firstResumeStarted = new Date(firstResumeTime);
        var firstResumeDate = firstResumeStarted.getTime()
